...
# curl -sfL https://get.outpost.run/setup.sh | sh -
...
select_network_interface() {
  if [ -z "$INSTALL_K3S_INTERFACE" ]; then
    ...
    echo "*******************************************************************************************************"
    echo ""
    echo "If you plan to use Outpost in a cluster using multiple servers in multiple Networks (private/public),"
    echo "choose the network Interface you want to use for the communication between the servers."
    echo ""
    echo "If you plan to use Outpost in a single server setup, choose the network Interface with the public IP."
    echo ""
...
wait_until_all_pods_running() {
  ...
  while true; do
    ...
    # Checks if there are no resources found --> Kubernetes is still starting up
...
# deploy Outpost
cat <<EOF >outpost-setup-job.yaml
apiVersion: v1
kind: Namespace
metadata:
  name: outpost
...
spec:
  ...
  template:
    spec:
      ...
      - name: outpost-container
        image: ghcr.io/outpostlabs/outpost:latest
        env:
        - name: START_MODE
          value: "setup"
        - name: K3S_JOIN_TOKEN
  ...
  backoffLimit: 0
EOF
sudo kubectl apply -f outpost-setup-job.yaml
rm outpost-setup-job.yaml
wait_until_all_pods_running
sudo kubectl logs -f job/outpost-setup-job -n outpost
...
# echo "curl -sfL https://get.outpost.run/setup-worker.sh | K3S_URL=https://<IP-ADDRESS-OR-HOSTNAME-OF-MASTERNODE>:6443 JOIN_TOKEN=$joinTokenForOtherNodes sh -"
